...
func (p *eventPublisherStub) Publish(event events.Event) {
...
func TestTaskServicePublishesOnlyCommittedEvent(t *testing.T) {
	publisher := &eventPublisherStub{}
	repository := &fakeTaskRepository{createID: 10}
	service := NewTaskService(repository, NewTaskPolicy(fakeTeamAccess{members: map[int64]bool{1: true}}), nil, publisher)

	if _, err := service.Create(context.Background(), 1, models.Task{Title: "Backend", Status: "todo", TeamID: 5}); err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	if len(publisher.events) != 1 || publisher.events[0].EventType != events.TaskCreated {
		t.Fatalf("published events = %+v, want task.created", publisher.events)
	}
	payload := publisher.events[0].Payload.(events.TaskPayload)
	if payload.Title != "Backend" {
		t.Fatalf("published task title = %q, want Backend", payload.Title)
	}

	publisher.events = nil
	repository.createErr = errors.New("commit failed")
	if _, err := service.Create(context.Background(), 1, models.Task{Title: "Failed", Status: "todo", TeamID: 5}); err == nil {
		t.Fatal("Create() error = nil, want failure")
	}
	if len(publisher.events) != 0 {
		t.Fatalf("events after failed create = %+v, want empty", publisher.events)
	}
}
