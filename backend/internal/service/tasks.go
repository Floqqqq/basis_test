...
func NewTaskService(tasks TaskRepository, policy *TaskPolicy, taskCache TaskCache, publishers ...EventPublisher) *TaskService {
	service := &TaskService{
		tasks:     tasks,
		policy:    policy,
		taskCache: taskCache,
...
func (s *TaskService) Create(ctx context.Context, userID int64, task models.Task) (taskID int64, err error) {
	...
	taskID, err = s.tasks.Create(ctx, task, func(taskID int64) events.Event {
		createdEvent = events.New(events.TaskCreated, userID, events.TaskPayload{
			TaskID:             taskID,
			TeamID:             task.TeamID,
			Title:              task.Title,
	...
	publishEvent(s.publisher, createdEvent)
...
func (s *TaskService) Update(ctx context.Context, userID, taskID int64, update TaskUpdate) (err error) {
	...
	for _, event := range domainEvents {
		publishEvent(s.publisher, event)
	}
