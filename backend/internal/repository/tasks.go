...
func (r *TaskRepository) Create(ctx context.Context, task models.Task, eventFactory func(int64) events.Event) (taskID int64, err error) {
	...
	if eventFactory != nil {
		if err := r.storeEvent(ctx, tx, eventFactory(taskID)); err != nil {
			return 0, err
		}
	}
...
func (r *TaskRepository) Update(ctx context.Context, userID int64, task models.Task, domainEvents []events.Event) (err error) {
	...
	for _, event := range domainEvents {
		if err := r.storeEvent(ctx, tx, event); err != nil {
			return err
		}
	}
...
func (r *TaskRepository) storeEvent(ctx context.Context, tx *sql.Tx, event events.Event) error {
	if err := r.outbox.Create(ctx, tx, event); err != nil {
		return err
	}
	if err := r.activity.Create(ctx, tx, event); err != nil {
		return err
	}
	return nil
}
