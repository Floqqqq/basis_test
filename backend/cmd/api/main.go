...
import (
	...
	"task-manager/internal/realtime"
...
func main() {
	...
	activityRepo := repository.NewActivityRepository(postgresDB)
	taskPolicy := service.NewTaskPolicy(teamRepo)
	taskCache := cache.NewTaskCache(redisClient, 5*time.Minute)
	realtimeHub := realtime.NewHub()
	teamService := service.NewTeamService(teamRepo, realtimeHub)
	taskService := service.NewTaskService(taskRepo, taskPolicy, taskCache, realtimeHub)
	activityService := service.NewActivityService(activityRepo, teamRepo)
	...
	realtimeHandler := realtime.NewHandler(realtimeHub, authService, teamRepo)
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Get("/teams/{id}/ws", realtimeHandler.Connect)
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/teams/{id}/activity", activityHandler.List)
